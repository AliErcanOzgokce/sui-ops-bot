...
6. `/review` -- Run a pre-landing review on the diff before every commit. Catch classification trust-boundary issues, sheet write-path bugs, and structural problems.

7. `/qa` -- After all issues for the feature are closed, run one full QA cycle to validate end-to-end behavior against the Build & Test Gate below.
...
6. Run the full test gate (see Build & Test Gate below). All checks must pass.
...
2. Run the full test gate (see Build & Test Gate below).
...
## Designing Something New (Product / Domain Model / Architecture)

> Use this before any code is written for a new capability, a new MCP tool, or a new classification behavior.
...
3. `/plan-eng-review` -- Lock in the technical plan before writing code. For classification or sheet-schema changes, this is where the taxonomy, column mapping, and human-in-the-loop flow are finalized.
...
2. `/autoplan` -- For thorough multi-pass review of a branch or plan. Use when the change is high-risk, cross-cutting, or needs an independent second opinion.
...
- Open the PR as a draft while work is in progress; mark ready for review only when `/review` and the test gate have passed.
- Use `/ship` to commit and open the PR in one step. Use `gh pr edit` if you need to update an existing PR description.

### Branch naming

Follow the pattern in `git-standards.md`: `<type>/<short-description>` (e.g. `feature/aging-report-filter`, `fix/sheet-column-drift`, `refactor/extract-report-builders`).

---

## Build & Test Gate

Before **shipping** (opening a PR or pushing to main), Claude MUST run the full test gate and verify everything passes. Passing lint alone is not sufficient; all tests and both config preflights must also pass.

For intermediate commits on a feature branch, the full test gate is NOT required. Commit early and often. The test gate applies before `/ship` and `/review`, not before every commit.
...
ruff check .
pytest -q
python -m sui_ops_bot.slackbot --check   # config preflight (no network side effects beyond token/sheet validation)
...
| Refactor with no behavior change            | `/simplify`                            |
| About to commit                             | `/review`                              |
| Feature is done, need full QA               | `/qa`                                  |
